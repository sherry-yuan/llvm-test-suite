//==------- lsc_slm_load.hpp - DPC++ ESIMD on-device test ------------------==//
//
...
bool test(uint32_t pmask = 0xffffffff) {
      ...
      cgh.parallel_for<KernelID<case_num>>(
          Range, [=](cl::sycl::nd_item<1> ndi) SYCL_ESIMD_KERNEL {
            ...
            if (ndi.get_local_id(0) == 0) {
              ...
              uint32_t group_off = groupID * slm_size_per_group;
              simd<T, gran> slm_val(new_val + groupID * slm_ud_per_group, 1);
            ...
            if constexpr (transpose) {
              auto vals = lsc_slm_block_load<T, VS, DS, L1H, L3H>(byte_off);
              ...
              auto loaded =
                  lsc_slm_gather<T, VS, DS, L1H, L3H, VL>(offset, pred);
  ...
  if constexpr (transpose) {
    ...
    for (int i = 0; i < Size; i++) {
      T e = (pmask >> ((i / VS) % VL)) & 1 ? (new_val + i) & vmask : old_val;
      if (out[i] != e) {
        passed = false;
        std::cout << "out[" << i << "] = 0x" << std::hex << (uint64_t)out[i]
                  << " vs etalon = 0x" << (uint64_t)e << std::dec << std::endl;
