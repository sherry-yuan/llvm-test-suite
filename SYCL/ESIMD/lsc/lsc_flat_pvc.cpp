//==------- lsc_flat_pvc.cpp - DPC++ ESIMD on-device test ------------------==//
//
      ...
      h.parallel_for<class SimplestKernel>(
          range<1>{size / SIMDSize}, [=](id<1> id) SYCL_ESIMD_KERNEL {
            ...
            lsc_prefetch<int, SIMDSize, lsc_data_size::default_size,
                         cache_hint::uncached, cache_hint::uncached>(vec_0 +
                                                                     offset);
            ...
            lsc_prefetch<int, 1, lsc_data_size::default_size,
                         cache_hint::uncached, cache_hint::uncached>(vec_1,
                                                                     offsets);
            auto data_1 = lsc_gather<int>(vec_1, offsets);
            lsc_scatter<int>(vec_1, offsets, data_1 * 2);

            lsc_atomic_update<int, atomic_op::inc>(vec_2, offsets, pred);
            lsc_atomic_update<int, atomic_op::add>(vec_3, offsets, add, pred);
            lsc_atomic_update<int, atomic_op::cmpxchg>(vec_4, offsets, compare,
                                                       swap, pred);
          });
    });
    q.wait();
